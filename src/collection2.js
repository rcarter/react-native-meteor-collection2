...
//Get a handle for the collection for easy export
const { Collection } = Mongo;

/**
 * Here we patch things that collection2 needs to run but we don't have access to in this environment.
 * The intent is to be able to have collection2 be a direct copy from the atmosphere package so when it
 * changes, we can bring in those changes easily.
 */
...
//We patch these here since you may share a schema with meteor code that has these schema keys
...
//Below this line should be an exact copy of collection2 except the export af the very bottom
